...
import com.lepu.anxin.utils.ByteArrayKt;
import com.lepu.anxin.vals.RunVarsKt;

import java.io.UnsupportedEncodingException;
import java.util.Calendar;

import static com.lepu.anxin.socket.objs.SocketMsgConst.ecgQueue;
import static com.lepu.anxin.socket.objs.SocketMsgConst.getEr1Config;
import static com.lepu.anxin.socket.objs.SocketMsgConst.getKcaConfig;
import static com.lepu.anxin.socket.objs.SocketMsgConst.getOxiConfig;
import static com.lepu.anxin.socket.objs.SocketMsgConst.getStatus;
import static com.lepu.anxin.socket.objs.SocketMsgConst.kcaQueue;
import static com.lepu.anxin.socket.objs.SocketMsgConst.oxyQueue;
...
//        LogUtils.d("模块状态: ", ByteArrayKt.toHex(msg.toBytes()));
    ...
    public static byte[] uploadKcaResult(KcaBleResponse.KcaBpResult result) {
        ...
        content[10] = toBcd(c.get(Calendar.YEAR) / 100);
        content[11] = toBcd(c.get(Calendar.YEAR) % 100);
        content[12] = toBcd(c.get(Calendar.MONTH) + 1);
        content[13] = toBcd(c.get(Calendar.DAY_OF_MONTH));
        content[14] = toBcd(c.get(Calendar.HOUR_OF_DAY));
        content[15] = toBcd(c.get(Calendar.MINUTE));
        content[16] = toBcd(c.get(Calendar.SECOND));
//        content[17] = (byte) state;
        ...
        LogUtils.d(ByteArrayKt.bytesToHex(content));
